...
<body>
  <div class="container">
    <header>
      ...
      <div class="controls">
        <div class="control">
          <label for="month" class="muted">Maand</label>
...
let isAdvancingMonth = false;
    ...
    function loadView(){ return metaState.view ? { ...defaultView(), ...metaState.view } : defaultView(); }
...
if(inv){ inv.textContent = `Beleggen-categorie: ${fmt.format(savingsBeleg)}`; }
      ...
      if(nwWrap){
        ...
        // Neem in Huidig totaal slechts de meest recente Overdracht mee (anders tel je historiek dubbel)
        let latestTransferAmt = 0;
        if (transfers.length) {
          const latest = transfers.reduce((acc, cur)=>{
            // label is "Overdracht YYYY-MM" → vergelijk lexicografisch op YYYY-MM
            const lm = String((acc.label||'').split(' ')[1]||'');
            const cm = String((cur.label||'').split(' ')[1]||'');
            return (cm > lm) ? cur : acc;
          });
          latestTransferAmt = num(latest.amount);
        }
  ...
  return; // niet verder verwerken
      ...
      if (field.startsWith('saving-')) {
        ...
        if(field==='saving-amount'){
          ...
          // Bepaal wat er nog over is na sparen (beleggen telt niet mee in projectie)
          const totalsNow = calculateTotals();
          const sparenSum = state.savings
            .filter(s => (s.kind || 'sparen') === 'sparen')
            .reduce((s, it) => s + num(it.amount), 0);
          const availableForInvest = totalsNow.net - sparenSum;

          if((item.kind === 'beleggen') && proposed > 0 && availableForInvest <= 0){
            // Geen geld meer over om te beleggen → reset bedrag en toon melding
            item.amount = 0;
            t.value = '0';
toast('Je hebt deze maand niets meer over om te beleggen. Verlaag je uitgaven of je spaarbedrag, of verhoog je inkomsten.');            render();
            return;
          }
        ...
        if(field==='saving-kind'){
          // Als je wisselt naar beleggen terwijl er geen geld meer is, reset bedrag
          if(t.value === 'beleggen'){
            const totalsNow = calculateTotals();
            const sparenSum = state.savings
              .filter(s => (s.kind || 'sparen') === 'sparen' && s.id !== item.id)
              .reduce((s, it) => s + num(it.amount), 0);
            const availableForInvest = totalsNow.net - sparenSum;
            if(availableForInvest <= 0 && num(item.amount) > 0){
              item.amount = 0;
toast('Je hebt deze maand niets meer over om te beleggen. Het bedrag is teruggezet naar 0.');            }
          }
          item.kind = t.value;
        }
        render();
      ...
      if(btn.id==='nextMonth'){
        ...
        try {
          ...
          // ===== Net worth overdracht (zet projectie van huidige maand als startbedrag voor volgende maand) =====
          {
            ...
            // Only the latest transfer counts toward the current base, same as UI
            ...
            // Idempotent: remove existing transfer for the current month, then add the new one
          ...
          if(investSum > 0){
            const meta = loadMeta();
            // verhoog totaal belegd
            setFutureInvestments(getFutureInvestments() + investSum);
            // update (of maak) het net worth item 'Future inkomsten (Beleggen)'
          ...
          try{
            const metaGoals = loadMeta();
            if (metaGoals && metaGoals.goalsAutoDistribute) {
              const totalToDistribute = tmpFutureSparen; // enkel sparen (geen beleggen)
              if (totalToDistribute > 0 && Array.isArray(metaGoals.goals) && metaGoals.goals.length) {
                // Neem enkel doelen die nog niet bereikt zijn
                const openGoals = metaGoals.goals
                ...
                if (openGoals.length) {
                  ...
                  // Eerste pass: proportioneel volgens prioriteit, met bovengrens = resterende behoefte
                  const adds = new Map();
                  openGoals.forEach(g => {
                    const need = Math.max(0, g.target - g.saved);
                    const share = (totalToDistribute * prioWeight(g.prio)) / totalWeight;
                    const add = Math.min(need, Math.max(0, Math.round(share * 100) / 100)); // afronden op centen
                    adds.set(g.id, add);
                  });
                  let used = 0; for(const v of adds.values()) used += v;
                  remaining = Math.max(0, Math.round((totalToDistribute - used) * 100) / 100);

                  // Tweede pass: restjes verdelen vanaf hoogste prioriteit
                  if (remaining > 0) {
                  ...
                  // Toepassen op doelen
                  metaGoals.goals = metaGoals.goals.map(g => {
